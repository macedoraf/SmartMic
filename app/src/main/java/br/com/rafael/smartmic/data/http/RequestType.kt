...
sealed class RequestType(
    ...
    class Connect(ipAddress: String, ipPort: String, deviceId: String) :
        RequestType("message", "NAME", ipAddress, ipPort, deviceId)

    class Disconnect(ipAddress: String, ipPort: String, deviceId: String) :
        RequestType("command", "GUEST_DISCONECTED", ipAddress, ipPort, deviceId)
    ...
    class UnmuteMic(ipAddress: String, ipPort: String, deviceId: String) :
        RequestType("command", "MIC_UNMUTED_BY_GUEST", ipAddress, ipPort, deviceId)

    class DeclinePool(ipAddress: String, ipPort: String, deviceId: String) :
        RequestType("response", "DECLINED_POLL", ipAddress, ipPort, deviceId)

    class StartPoolAck(ipAddress: String, ipPort: String, deviceId: String) :
        RequestType("response", "POLL_START_ACK", ipAddress, ipPort, deviceId)

    class PollAwnser(
        um: String, dois: String, ipAddress: String, ipPort: String,
        val deviceId: String
    ) :
        RequestType("response", "POLL_ANSWER", ipAddress, ipPort, deviceId, um, dois)
